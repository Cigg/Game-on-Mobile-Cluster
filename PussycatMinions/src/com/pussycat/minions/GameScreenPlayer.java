...
public class GameScreenPlayer extends Screen {
    		  ...
    		  if(event.pointer >= 2) {
    			...
    			switch(SharedVariables.getInstance().getInternalState()) {
    				...
    				case ADD_DEVICE:
    				{
    		    		...
    		    		buffer.putShort((short) 1);										// type, 0 �r h�rdkodat till main-device - s�tt 1 f�r alla andra devices
	...
	// F�r trace/trigger - ska fixa fint
	// TODO: ska fixa fint och st�da
        	...
        	for(int i = beginIndex ; i<index ; i += jump) {
        		...
        		if( i > 7 ) {
	        		...
	        		for(int j=0 ; j<steps; j++) {
	        			...
	        			scale = j / (float) steps;  // Kan r�knas ut i f�rv�g
 ...
 // TODO - Centrera bilden och texten.
    private void drawSharedUI(GameState state) {
    	...
    	if (state == GameState.AddDevice) {
    		...
    		g.drawString("Klicka f�r att g� med i spelet", textX, textY, paint);
    	} else if (state == GameState.NotMapped) {
    		g.drawImage(Assets.ball, imageX, imageY);
    		g.drawString("Lyckost! Det �r din tur att mappa nu.", textX, textY, paint);
    	} else if (state == GameState.Wait) {
    		g.drawImage(Assets.ball, imageX, imageY);
    		g.drawString("V�nta! N�gon annan mappar.", textX, textY, paint);
    	} else if (state == GameState.MappedWait) {
    		g.drawImage(Assets.ball, imageX, imageY);
    		g.drawString("V�nta medans de andra mappar sina enheter", textX, textY, paint);
